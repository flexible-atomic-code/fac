...
\newcommand{\facversion}{{1.0.1}\xspace}
\newcommand{\opt}[1]{
...
\label{sec:pol}
\index{pol}
This module is used to calculate line polarizations due to electron
collisional excitation. It takes into account the radiative cascades effects
...
\begin{fundesc}{PolarizationTable}{fn}
Calcuates the line polarizations, and output the results to file \var{fn} in a
simple ASCII format. It contains 7 columns, which are upper level index, lower
level index, multipole type (as in the output of \key{TransitionTable}),
transition energy in eV, total line emissivity, emission anisotropy factor at
$90^\circ$, and linear polarization, respectively.
\end{fundesc}
...
it tabulates the total population and magnetic sublevel fractions.
\end{fundesc}
...
\begin{fundesc}{SetEnergy}{e, s}
Set the electron energy \var{e} in eV. if \var{s} is positive, the energy
distribution is a Gaussian with standard deviation $\sigma=s$ in eV, and mean
$e$.
\end{fundesc}
