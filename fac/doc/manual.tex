...
\newcommand{\facversion}{{1.0.4}\xspace}
\newcommand{\opt}[1]{
...
The record for each level contains 9 fields of the type: short, int,
int, short, short, short, double, double, double. They are number of electrons
of the ion, block index of the level, level index within the block, 2J value
of the level, base level of the level, valence $nl$ of the level, energy,
total decay rate, and branching ratio computed in \key{DRBranch}
respectively. 
\item[$m=1$]
...
levels that are not explicitly calculated. Default is 1, enabling the
extraploation.
\end{fundesc}
