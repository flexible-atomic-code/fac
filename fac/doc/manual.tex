...
\newcommand{\facversion}{{0.9.0}\xspace}
\newcommand{\opt}[1]{
...
Calculate the Seaton's $y5$ function for negative energies. This is
basically the Coulomb wavefunction with principal quantum number \var{n} and
orbital angular momentum \var{l} at radius \var{r}. It returns a tuple (y5,
y5p, n1, n2, err), where y5 is the value, y5p is the derivative. n1 is the
ln of a normalization factor which, multiplied with y5, gives the Wittaker
function. n2 is the ln of an additional normalization factor which,
mutiplied with Wittaker function, gives the normalized Coulomb wavefunction.
err is an error code returned by coulcc, which is called internally to do
the calculation.
\end{fundesc}
...
Calculate the direct ionization rates using the Fortran subroutine \verb|cfit|
of D. A. Verner. \var{Z} is the nuclear charge of the ion, \var{n}
is the number of electrons, and \var{Te} is the electron temperature in eV.
The function returns double as the result in unit of 
$10^{-10}$ cm$^3$ s$^{-1}$. 
\end{fundesc}
...
\key{ColFit} for the autoionization rate, if \var{m} is 2. The default is 1.
...
FAC for H-like through Ne-like ions of Mg, Si, S, Ar, Ca, Fe, and Ni, and the
fitting formula and data table of P. Mazzotta for other ions. \var{Z} is the
nuclear charge of the ion, \var{n} is the number of electrons, and \var{Te} is
the electron temperature in eV. The result is in unit of $10^{-10}$ cm$^3$
s$^{-1}$.
\end{fundesc}
...
F-like ions of Mg, Si, S, Ar, Ca, Fe, and Ni, and use the routine \verb|rrfit|
for other ions. \var{Z} is the nuclear charge of the
