...
out. If ``n == 0'', all levels are outpup. ``m'' indicates whether the mixing
coefficients should be printed out. The defautls: ``n = 0'', ``m = 0''.
...
\begin{description}
\item[SetCEPWOptions(max_allow, max_forbidden, max_f, qr, tol)] 
...
wavefunction and treat the orbitals with differetn $j$ but same $l$ as the
same. ``max_allow'' is the maximum partial wave to be included for dipole
allowed transitions, ``max_forbidden'' is that for forbidden transitions,
``max_f'' is a factor to be multiplied to e/delta_e to produce a limit on the
maximum partial wave. Internally, a partial wave grid is setup from $l = 0$ to 
``max'', which includes every $l$ below 5 but use a logrithmic grid beyond
...
automatically according to the output grid. 
...
refer to the parrel routines in the excitation for the syntax.
\begin{description}
\item[SetCIPWOptions(max_allow, max_forbidden, max_f, qr, tol)] 
...
states constructed this way can be referred to by a tuple (n, groupnames). 
...
calculated if ``ai'' is accessable from ``g''.
...
ocuur, e.g., [0], if only DR on to the ground level is to be
...
number to which the decay of autoioning levels are allowed. ``nspec'' is the
...
to nmax, extrapolation using $n^{-3}$ scaling law.
\end{description}
