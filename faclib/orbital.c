...
static char *rcsid="$Id: orbital.c,v 1.55 2004/05/27 15:55:00 mfgu Exp $";
#if __GNUC__ == 2
...
int RadialBasis(ORBITAL *orb, POTENTIAL *pot) {
  double z, z0, e, de, ep, emin, emax;
  double *p, norm2, fact, p1, p2, qo, bqp;
...
int RadialBound(ORBITAL *orb, POTENTIAL *pot) {
  ...
  while (niter < max_iteration) {
    ...
    IntegrateRadial(p, e, pot, i2m2, p1, pot->maxrp-1, 0.0, NULL);
...
int DiracSmall(ORBITAL *orb, POTENTIAL *pot) {
  ...
  if (orb->n > 0) {
    if (pot->ib > 0 && orb->n > pot->nb) i1 -= 2;
    for (i = i1; i < pot->maxrp; i++) {
      p[i] = 0;
      p[i+pot->maxrp] = 0.0;
    }
    a = InnerProduct(0, i1-1, p+pot->maxrp, p+pot->maxrp, pot);
    b = InnerProduct(0, i1-1, p, p, pot);
    a *= orb->qr_norm;
    b *= orb->qr_norm;
    a = sqrt(a+b);
...
int SetOrbitalRGrid(POTENTIAL *pot, double gratio, double gasymp) {
  int i;  
  ...
  nmax = sqrt(rmax)/2.0;
  if (nmax < 5) {
    printf("rmax not large enough, enlarge maxrp.\n");
    exit(1);
  }
  rmax /= z;
