...
static double _zd[12] = {1.0, 10.0, 20.0, 30.0, 40.0, 50.0, 
			 60.0, 70.0, 80.0, 90.0, 100.0, 110};
static double _sd[16][12] = {{10.3168,4.6540,3.2460,2.5519,
...
static int _nd[5] = {0, 1, 4, 8, 12};
double HydrogenicSelfEnergy(double z, int n, int k, double rms) {
  ...
  r = 0;
  if (n <= 2 && z >= 26 && k != -2) {
    r = HydrogenicSelfEnergyLowN(z, n, k, rms*RBOHR*1e5);
  }
  if (r == 0) {
    if (n <= 5) {
      id = _nd[n-1];
