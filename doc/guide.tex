%% rcsid = $Id: guide.tex,v 1.16 2001/09/24 19:36:43 mfgu Exp $
...
from ``fac'' has names with each word capitalized in first letter. The modules
are grouped under the package ``pfac'', which stands for ``package fac''. The
import statement  
``from pfac import *'' makes all functions available to the script with module
name qualifiers, e.g., ``fac.SetAtom('Fe')''. Alternatively, one may use the
fully qualified names including the package name to avoid any possible name
space crash with other installed modules and packages. For example, the import
statement ``import pfac.fac'', and ``pfac.fac.SetAtom('Fe')''. 
...
\begin{description}
\item[SetCEPWOptions(eps, lmax, qr)] 
...
equal to ``eps''. The last 2 parameters are
optional. ``lmax'' sets an absolute limit for the partial wave 
summation, which may or may not be reached in the calculation. ``qr''
specifies $l_{qr}$, 
beyond which, the quasi-relativistic approximaiton should be used. The default
values for these parameters are, ``eps = 0.05'', ``lmax = 500'', and ``qr =
0''.  
...
\item[SetCEFormat(m)]
Set the format of the output. If ``m=0'', both collision strength and cross
sections in unit of $10^{-20}$ cm$^2$ are output. If ``m=1'', only collision
strenght is ouput, and if ``m=2'', only cross section is output.
...
\item[SetUsrCEGridType(type)]
Set the type of collision energy grid specified by ``SetUsrCEGrid''
``type = 0'', the energies are for the incident
electron, if it is 1, they are for the scattered electron, which is the
default.  
...
\begin{description}
\item[SetCIPWOptions(eps, lmax, lmax\_eject, qr)]
...
``eps=0.05'', ``lmax=500'', ``lmax\_eject=8'', ``qr=0''.
...
\item[SetCIFormat(m)]
Set the format of the output. If ``m=0'', both ionization strength and cross
sections in unit of $10^{-20}$ cm$^2$ are output. If ``m=1'', only ionization
strenght is ouput, and if ``m=2'', only cross section is output.
...
\item[SetUsrCIEGridType(type)]
Set the collision energy grid type specified by ``SetUsrCIEGrid'', if ``type =
0'', the energies are those of the incident electrons, if it is 1, the
energies are the sum of the two final electrons.
...
\item[SetUsrPEGridType(type)]
Set the energy grid type set by ``SetUsrPEGrid'', if ``type = 0'', the
energies are those of the incident photons, if it is 1, the energies are those
of the photo-electrons, which is the default. 
