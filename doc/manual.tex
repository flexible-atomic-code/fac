...
\newcommand{\facversion}{{1.0.8}\xspace}
\newcommand{\opt}[1]{
...
(in atomic units):
\begin{eqnarray}
\sigma_{PI} &=& 2\pi\alpha\frac{1+\alpha^2\varepsilon}
      {1+\frac{1}{2}\alpha^2\varepsilon}
                 \frac{d f}{d E} \nonumber\\
            &=& \frac{2\pi\alpha}{g_i}
...
parity. If \var{ilev}$<0$, it returns the index of the first level with number
...
transition rates output contains three additional fields, the energy shift,
gausian standard deviation, and the correction to the line strengths due to
...
argument \var{ci} indicates when reading the \key{DB\_TR} files, the
configuration interaction correction factors should be included. These
