...
\newcommand{\facversion}{{0.7.4}\xspace}
\newcommand{\opt}[1]{
...
performs a radial optimization for each configuration group, and calculate the
configuration average energy under each potential. The call with \var{m} = 1
recalculates the configuration average energy under the potential obtained by
\key{OptimizeRadial}, The energy of each level obtained by \key{Structure} is
...
the level energies introduced by using a single central potential for all configurations.
\end{fundesc}
...
units of eV, and are added to the energies calculated by FAC. If one needs to
bring the energy of certain level to a desired experimental value, the
calculated energy without correction must be known, presumably by a
calculation without calling \key{CorrectEnergy}.
\end{fundesc}
...
\var{q}$\ge$\var{t0} are selected. The physical meaning of line types are discussed in \S\ref{subsec:sp_header}
\end{fundesc}
...
the calculated wavelengths due to the cacellation. The accuracy for UV and
...
documentation and code comments. The documentation has been improving since
...
to be added to the list. If the list ever grows to the point when I can no
longer put it under my personal address book, we may have to create a true
mailing list.}
