...
\newcommand{\facversion}{{1.0.6}\xspace}
\newcommand{\opt}[1]{
...
\begin{fundesc}{CECross}{ifn, ofn, low, up, e}
...
to establish the energy table that is consistent with \var{ifn}
\end{fundesc}
...
interpolating the data from CE binary data file \var{ifn}. The results are
...
g, h, i, k, l, m, n, o, q, r, t, u, v, w, x, y, z} are available. 
\end{fundesc}
...
\begin{fundesc}{MBPT}{fn, g, gp, n0, n1, lmax\opt{, dn, mode}}
Calculate 2nd order correction to the energies using many-body perturbation
theory. Results are written to file \var{fn}. \var{g} is the group of levels
need corrections. \var{gp} is the group of configurations included as
perturbers. \var{n0} and \var{n1} specfifies the minimum and maximum principle
quantum numbers of the perturbers. \var{n0} may also be an array of ingegers
with the same size of gp, so that each group in gp can have different value
for the minimum principle quantum number. \var{lmax} is the maximum
orbital angular momentum of the perturbers. \var{dn} is the number of
extrapolating points added beyond \var{n1}, which are logarithmicly 
spaced. \var{mode} is the calculation mode, which should usually set to 2.
One should always call \key{SetBoundary} 
first to impose boundary conditions on the high-n orbitals, which can take
positive energies to represent the integration over the continuum. In this
method, only single and double excitations are taken into account in the
perturbation expansion. Configurations in \var{gp} can also be the core of the
perturbers without the highly excited orbitals, in which case, those orbitals
are added to the core configurations before the expansion is evaluated.
\end{fundesc}
...
minimization of the total energy of the mean configuration by adding a
correction to the radial potential. The correction is expressed as linear
superposion of $s$ orbitals with $n=1$--$5$. The coefficients are obtained by
minimization of the energy. \var{n} is the number of energy evaluations
allowed in the minimization, and \var{m} controls the print out during the
calculation. Default: \var{n} = 100, \var{m} = 0 (no print out).
\end{fundesc}
...
\begin{fundesc}{SetBoundary}{n, b\opt{, p}}
Set the boundary condition for high-n orbitals, so that they can represent the
continuum. \var{n} is the maximum principle quantum number represents the
...
\begin{fundesc}{SetCEBorn}{x}
Set the energy boundary between the distroted-wave Born approximation and
plane-wave Born approximation for the collisional excitation. If the scattered
electron energy is less than $xE_{b}$, DW method is chosen, where $E_{b}$ is
the binding energy of the electron being excited and $x$ is the positive. 
...
partial-waves calculated explicitly. If $x=0$, then DW is always used.
default for $x$ is -0.35, i.e., use DW when the estimated high partial-wave
contributions represents no more than 25\% of the explicitly calculated low
partial-wave sum.
\end{fundesc}
...
\begin{fundesc}{SetRadialGrid}{n\opt{, r0\opt{,r1}}}
...
oscillations of very high-$n$ orbitals at large radii, which approximately
scales as $\sqrt{r}$.
\end{fundesc}
...
\begin{fundesc}{StructureMBPT}{fn1, fn2, g0, g1, n0, ni, n1, k, nt, n2, nt2, gn\opt{,
x1, x2}} 
Solve the atomic structure with 2nd order many-body purterbation
corrrections. \var{fn1} is the file name for binary energy output. \var{fn2}
is the file containing the MBPT energy corrections. \var{g0} is the
configuration groups whose energies are to be solved. \var{g1} is a list of
configuration groups to be included as the virtual states in the MBPT
correction. \var{n0} specifies the minimum $n$ of the excited orbitals in the
virtual states. If it is a list, each element corresponds to one configuration
group in the \var{g1} list. If it is a number, each group in \var{g1} list has
the same number given by \var{n0}. \var{ni} is the maximum $n$ value of the
virtual states to be considered for all order inclusion. \var{n1} is the
maximum $n$ of the virtual states. \var{k} is the maximum $l$ value of the
virtual states. \var{nt} is the number of extra $n$ values to be added to the $n$
values beyond those given by \var{n1}. \var{n2} and \var{nt2} are similar
parameters for the second excited orbital. \var{gn} is a string specifies the
configuration group name for configurations included in the all order
calculation. \var{x1} is a small number specifying the threshold for all-order
calculation. If \var{x1} is negative, then no extra configurations other than
those in \var{g0} is included in the all order calculation. \var{x2} specifies
the minimum mixing amplitude to be included in the MBPT summation.
\end{fundesc}
...
assumed to be 0, and if \var{imax}$<$0, it is assumed to be the larges index
...
Calculate the ionization cross sections using the Aladdin data base. data were
...
\begin{fundesc}{RateTable}{fn\opt{, cfg}}
...
which is a list of complex names. All states in the specified complexes must
have a single data block in the output file, even if these states are within
some superlevel block. Otherwise, each superlevel block has a single data
block in the \key{DB\_RT} file.
\end{fundesc}
