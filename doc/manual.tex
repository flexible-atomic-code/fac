...
\newcommand{\facversion}{{1.0.2}\xspace}
\newcommand{\opt}[1]{
...
\label{sec:binary}
\index{Binary format}
Presently, FAC produces 10 different types of files. Each type is asigned
a unique integer from 1--10, which corresponds to a macro define in the file
...
\begin{fundesc}{GetGOSTail}{}
Return the maximum ratio of the generalized oscillator strengths at the
largest momentum transfer to its maximum. This number should be generally
small in order for the plane-wave Born approximation of collisional
excitation to be accurate. 
\end{fundesc}
...
then the Breit interaction involving all bound continuum states is
...
default for $x$ is -0.25, i.e., use DW when the estimated high partial-wave
...
\begin{fundesc}{SetGOSLimits}{max\opt{, min}}
Set the collision energy corresponding to the maximum and minimum momentum
transfer grid for generalized oscillator strenghth. These numbers are
multiplied by the maximum transition energy of the transition array to produce
...
with principal quantum number \var{n} and orbital angular momentum \var{l} at
...
eV, and mean $e$. This is only used for resonances. For direct excitation, a
mono-energetic beam is always assumed.
\end{fundesc}
